<template>
  <SwCard title="Context">
    <p>
      You can access several information about the current context with the
      Context API from the SDK. Here are some examples:
    </p>
    ...
    <p>
      Most information can also be subscribed to. So you react dynamically to
...
async function getCurrentModuleInformation() {
  ...
  notification.dispatch({
    ...
    <br /> <br />
    ${currentModuleInformation.modules
      .map((module, index) => {
        return `
        <b>
          Module ${index + 1}:
