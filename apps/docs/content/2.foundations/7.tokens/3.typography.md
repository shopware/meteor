---
title: Typography
description: Meteor uses Inter as its single typeface, with all type sizes, weights, and line heights available as CSS custom properties and via the Text component.
---

Meteor uses Inter as its single typeface. All type sizes, weights, and line heights are available as CSS custom properties and via the text component.
...
Use the text component to render text. It accepts `size`, `weight`, `color`, and `as` props and applies the correct token combination automatically.
...
::do-dont{do-text="Use font-size and line-height tokens together from the same scale step." dont="Mix a token font-size with a hardcoded line-height, or use raw px values."}
::

::do-dont{do-text="Use --font-weight-semibold or --font-weight-bold only for headings and labels." dont="Apply bold weight to long-form body text. It reduces readability."}
::
