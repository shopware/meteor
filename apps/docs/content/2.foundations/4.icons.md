---
description: Meteor icons follow a minimal, expressive style built for consistency, clarity, and legibility across all sizes and themes.
---

::page-header{source-path="packages/icon-kit" npm-package="@shopware-ag/meteor-icon-kit"}
::

Meteor icons follow a minimal, expressive style aligned with Shopware's product language. The set is built for consistency, clarity, and legibility across all sizes and themes. For technical usage, use the `mt-icon` component from the component library.
...
The searchable icon browser from Storybook will move here in a later migration pass. Until then, use the icon kit package as the source of truth for available icon names.
...
::do-dont{do-text="Use 24px, 20px, and 16px for the vast majority of icons. Reserve smaller sizes for genuinely dense UI." dont="Use arbitrary sizes. The common size set keeps the interface visually consistent."}
::
...
See the [Tokens](/foundations/tokens/tokens-overview) page for the full list of available icon color tokens.

::do-dont{do-text="Use icon color tokens so icons respond to theme changes and state transitions automatically." dont="Hardcode hex values for icon color. They will break in dark mode and won't reflect interactive states."}
::
...
::do-dont{do-text="Support icons with nearby text or an existing control label." dont="Rely on an icon alone to communicate a critical action or state. Color and shape alone are not sufficient."}
::
