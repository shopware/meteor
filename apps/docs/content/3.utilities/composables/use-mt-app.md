...
`useMtApp` gives any component below [**App**](/components/app) access to the shell: whether the mobile layout is active, which drawer is open and the theme, plus functions to open or close drawers and to set the theme. Outside of a shell it returns inert defaults and warns once in development, so components that use it keep working on their own.
...
| `openDrawer`    | `(side: "start" \| "end") => void`        | Opens the drawer of the given side. Does nothing in the desktop layout or for an empty sidebar.        |
...
- [**App**](/components/app)
- [**useTheme**](/utilities/composables/use-theme)
