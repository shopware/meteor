...
Replace custom CSS variables or hard-coded colors with [Meteor tokens](/design/tokens):
...
The component name is the same. The tag just changes from `sw-icon` to `mt-icon`. See the [Icons](/design/icons) page for the full icon set and usage guidance.
...
For the full API and usage guidance, see the [Admin SDK documentation](https://developer.shopware.com/resources/admin-extension-sdk/).
...
When upgrading between major versions of Meteor packages, check the [changelog](/getting-started/changelog) for breaking changes. Each release includes a changelog entry that lists renamed props, removed components, and token renames.
