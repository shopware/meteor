...
For a standalone application, make [**App**](/components/app) the root of your component tree. It lays out the header, both sidebars and the content area, turns the sidebars into drawers below 1280px, applies the color theme, mounts the [**Snackbar**](/components/snackbar) host and enables all future flags, so it replaces the [**Theme Provider**](/utilities/components/theme-provider) wrapper.
...
Some behavior that will become the default in upcoming major releases is available early behind future flags. [**App**](/components/app) enables all of them by default. Without the shell, opt in through the `future` prop of the [**Theme Provider**](/utilities/components/theme-provider) to align your application with the next major and reduce the work when you upgrade.
