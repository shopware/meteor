...
For a standalone application, make [**App**](/components/app) the root of your component tree. It lays out the header, both sidebars and the content area, turns the sidebars into drawers below 1280px, applies the color theme, mounts the [**Snackbar**](/components/snackbar) host and provides future flags, so it replaces the [**Theme Provider**](/utilities/components/theme-provider) wrapper.

```vue
<template>
  <MtApp :future="{ all: true }" v-model:theme="theme">
...
::note
The shell claims the full viewport and positions its drawers against it. Use it for applications that own the browser window. Administration extensions, apps rendered inside iframes and components embedded into an existing page layout keep using the [**Theme Provider**](/utilities/components/theme-provider) and their host's layout instead.
::

### Future flags

Some behavior that will become the default in upcoming major releases is available early behind future flags. Opt in through the `future` prop of [**App**](/components/app) or, without the shell, of the [**Theme Provider**](/utilities/components/theme-provider) to align your application with the next major and reduce the work when you upgrade.
