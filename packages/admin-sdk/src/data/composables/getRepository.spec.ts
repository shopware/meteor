...
describe("getRepository", () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    (createRepositoryAdapter as jest.Mock).mockReturnValue(
      mockRepositoryAdapter
    );
    (repositoryModule.default as jest.Mock).mockReturnValue(
  ...
  describe("with propRepositoryFactory", () => {
    it("should use the provided repository factory", () => {
      const propRepositoryFactory = jest
        .fn()
        .mockReturnValue(mockRepositorySource);

      const repository = getRepository("product", propRepositoryFactory);

      expect(propRepositoryFactory).toHaveBeenCalledWith("product");
      expect(createRepositoryAdapter).toHaveBeenCalledWith(
  ...
  describe("with lazy loading fallback", () => {
    ...
    it("should pass all arguments to the underlying repository methods", async () => {
      ...
      await repository.search(
        { limit: 10 } as Criteria,
        { languageId: "en-US" } as ApiContext
      );
      ...
      await repository.save(
        { id: "123", name: "Test" } as Entity<"product">,
        { languageId: "en-US" } as ApiContext
      );
