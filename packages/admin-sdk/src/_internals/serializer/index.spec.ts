...
describe("index.ts", () => {
  it("should convert entities with circular reference", () => {
    ...
    expect(
      result.entity.__draft__.lineItems.__entities__[0].__draft__.children
        .__entities__[0] instanceof Entity,
    ).toBe(false);
    expect(
      result.entity.__draft__.lineItems.__entities__[0].__draft__.children
        .__entities__[1] instanceof Entity,
    ).toBe(false);
    expect(
      result.entity.__draft__.lineItems.__entities__[0].__draft__.children
        .__entities__[0].__draft__.parent.children instanceof EntityCollection,
    ).toBe(false);
    expect(
      result.entity.__draft__.lineItems.__entities__[0].__draft__.children
        .__entities__[0].__draft__.parent,
    ).toStrictEqual({
    ...
    expect(
      deserialized.entity.lineItems.getAt(0).children.getAt(0).parent,
    ).toBe(deserialized.entity.lineItems.getAt(0));
  });
});
