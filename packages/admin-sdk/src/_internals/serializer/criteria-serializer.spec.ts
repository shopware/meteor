...
describe("Criteria Serializer", () => {
  ...
  it("should serialize the criteria with correct values", () => {
    ...
    myCriteria.addFilter(Criteria.equals("name", "Hello world"));

    myCriteria.addFilter(
      Criteria.multi("and", [
        Criteria.equals("defaultFolderId", "exampleDefaultFolderId"),
        Criteria.not("or", [Criteria.equals("id", "exampleFolderId")]),
      ]),
    );

    myCriteria.setIds(["idOne", "idTwo"]);

    myCriteria.addQuery(Criteria.contains("group.name", "exampleGroup"), 500);
    ...
    expect(deserializedMessageData.myCriteria.parse()).toEqual(
      originalParsedCriteria,
    );
  });
});
