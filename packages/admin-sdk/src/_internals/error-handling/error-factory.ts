...
export default function createError(
  ...
  /* eslint-disable */
  if (
    hasOwnProperty(e, "response.data.errors.0.code") &&
    (e as any).response.data.errors.length
  ) {
    const missingPrivilegeErrors = (e as any).response.data.errors.filter(
      (error: { code: string }) =>
        error.code === "FRAMEWORK__MISSING_PRIVILEGE_ERROR",
    ) as { detail: string }[];

    const missingPrivileges: privilegeString[] = [];

    missingPrivilegeErrors.forEach((mpe) => {
      const data = JSON.parse(mpe.detail) as {
        message: string;
        missingPrivileges: privilegeString[];
      };
    ...
    return new HandleError(
      (e as any).response.data.errors[0].code,
      (e as any).response.data.errors[0].status,
    );
  }
