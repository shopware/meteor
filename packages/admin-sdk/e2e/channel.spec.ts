...
test.describe("Main communication test", () => {
  ...
  test("get value from mainFrame with additional data", async ({ page }) => {
    ...
    await Promise.all(
      testMatrix.map(async (testScenario) => {
        expect(
          await subFrame.evaluate(async ([firstNumber, secondNumber]) => {
            return await window.sw_internal.send("_multiply", {
              firstNumber,
              secondNumber,
            });
          }, testScenario),
        ).toEqual(testScenario[2]);
      }),
    );
  });
  ...
  test("call method in subFrame from mainFrame", async ({ page }) => {
    ...
    // @ts-expect-error
    const methodOneWasCalled = await subFrame.evaluate(
      () => window.methodOneWasCalled,
    );
    // @ts-expect-error
    const methodTwoWasCalled = await subFrame.evaluate(
      () => window.methodTwoWasCalled,
    );
...
test.describe("Context tests", () => {
  test("get language", async ({ page }) => {
    ...
    const { systemLanguageId, languageId } = await subFrame.evaluate(
      async () => {
  ...
  test("subscribe language", async ({ page }) => {
    ...
    await subFrame.evaluate(async () => {
      return await window.sw.context.subscribeLanguage(
...
test.describe("Serializing tests", () => {
  test("send criteria to iFrame", async ({ page }) => {
    ...
    const result = await subFrame.evaluate(async () => {
      ...
      return {
        ...
        isCriteriaInstance:
          result.myCriteria instanceof window.sw_internal.Criteria,
      };
    });
  ...
  test("send entity collection to iFrame", async ({ page }) => {
    ...
    const result = await subFrame.evaluate(async () => {
      ...
      collection.add(
        new window.sw_internal.Entity("2", "test", {
      ...
      return {
        ...
        isCollectionInstance:
          result.collection instanceof window.sw_internal.Collection,
        // @ts-expect-error
        nestedEntityIsEntity:
          typeof result.collection[1].foo.getDraft === "function",
        // @ts-expect-error
        nestedCollectionIsCollection:
          result.collection[1].foo.anotherCollection instanceof
          window.sw_internal.Collection,
      };
    });
  ...
  test("send empty entity collection in entity to iFrame", async ({ page }) => {
    ...
    const result = await subFrame.evaluate(async () => {
      ...
      return {
        ...
        // @ts-expect-error
        anotherCollectionIsCollection:
          result.entity.collection.get("2").foo.anotherCollection instanceof
          window.sw_internal.Collection,
        // @ts-expect-error
        originCollectionIsCollection:
          result.entity.collection.get("2").foo.getOrigin()
            .anotherCollection instanceof window.sw_internal.Collection,
      };
    });
  ...
  test("should not mutate original data when serializing", async ({ page }) => {
    ...
    const result = await subFrame.evaluate(async () => {
      ...
      collection.add(
        new window.sw_internal.Entity("1", "test", {
          foo: "bar",
        }),
      );
      ...
      const result = await window.sw_internal.send(
        "_collectionTest",
        sendObject,
      );
...
test.describe("Privilege tests", () => {
  test("promise rejection with string", async ({ page }) => {
    ...
    const result = await subFrame
      .evaluate(async () => {
        return await window.sw_internal.send("repositorySearch", {
          entityName: "product",
        });
      })
      .catch((e) => e);
  ...
  test("promise rejection with error", async ({ page }) => {
    ...
    expect(
      result.message.includes(
        'Your app is missing the privileges product:read for action "repositorySearch".',
      ),
    ).toBe(true);
  });

  test("should not accept entity data without correct privileges (create,read,update,delete)", async ({
    page,
  }) => {
    const { mainFrame, subFrame } = await setup({ page });
    ...
    await mainFrame.evaluate(() => {
      ...
      window.sw_internal.handle("_collectionTest", () => {
        ...
        collection.add(
          new window.sw_internal.Entity("productEntityId", "product", {
            name: "Amazing T-Shirt",
            foo: new window.sw_internal.Entity(
    ...
    const response = await subFrame.evaluate(async () => {
      ...
      try {
        ...
        return {
          ...
          isMissingPrivilesErrorInstance:
            error instanceof window.sw_internal.MissingPrivilegesError,
        };
      }
    });

    expect(response.errorMessage).toEqual(
      `Error: Your app is missing the privileges create:product, delete:product, update:product, create:manufacturer, delete:manufacturer, read:manufacturer, update:manufacturer for action "_collectionTest".`,
    );
    expect(response.isMissingPrivilesErrorInstance).toBe(true);
  });

  test("should accept entity data without correct privileges when on the same origin (for plugins)", async ({
    page,
  }) => {
    const { mainFrame, subFrame } = await setup({ page });
  ...
  test("should not send entity data without correct privileges (read)", async ({
    page,
  }) => {
    const { mainFrame, subFrame } = await setup({ page });
    ...
    expect(response.errorMessage).toEqual(
      `Error: Your app is missing the privileges read:manufacturer for action "_collectionTest".`,
    );
    expect(response.isMissingPrivilesErrorInstance).toBe(true);
  });

  test("should not send entity data without correct privileges in data handling (read)", async ({
    page,
    browserName,
  }) => {
    const { mainFrame, subFrame } = await setup({ page });
    ...
    await mainFrame.evaluate(async () => {
      ...
      const exampleProduct = new window.sw_internal.Entity(
        "exampleProductEntityId",
        "product",
        {
          name: "T-Shirt",
    ...
    const result = await subFrame.evaluate(() => {
      return {
        // @ts-expect-error
        isError:
          window.result.data instanceof
          window.sw_internal.MissingPrivilegesError,
        // @ts-expect-error
    ...
    if (browserName !== "firefox") {
      ...
      expect(consoleError.text()).toContain(
        'Your app is missing the privileges read:product for action "datasetSubscribe".',
      );
    }
    ...
    expect(result.errorText).toBe(
      'Error: Your app is missing the privileges read:product for action "datasetSubscribe".',
    );
  });

  test("should send entity data with correct privileges in data handling (read)", async ({
    page,
  }) => {
    const { mainFrame, subFrame } = await setup({ page });
...
test.describe("data handling", () => {
  ...
  test("dataset subscriber with selector", async ({ page }) => {
    ...
    // subscribe to dataset publish
    await subFrame.evaluate(async () => {
      return await window.sw.data.subscribe(
        "product_detail",
        (data) => {
          // @ts-expect-error
          window.result = { data: data.data };
  ...
  test("dataset update", async ({ page }) => {
    ...
    const data = await mainFrame.evaluate(async () => {
      ...
      window.sw.data.updateSubscriber("e2e-test", (data) => {
        // @ts-expect-error
