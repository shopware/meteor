...
<StorybookPageHeader status="none" title="Accessibility" />
...
- **Color contrast**: [Design tokens](?path=/docs/foundations-tokens-overview--docs) meet WCAG 2.1 AA contrast ratios for text and UI elements
...
- Use the `decorative` prop on [`mt-icon`](?path=/docs/components-icon--docs) for purely visual icons so they stay out of the accessibility tree
