...
Meteor uses Inter as its single typeface. All type sizes, weights, and line heights are available as CSS custom properties and via the [Text](?path=/docs/components-text--docs) component.
...
Use the [Text](?path=/docs/components-text--docs) component to render text. It accepts `size`, `weight`, `color`, and `as` props and applies the correct token combination automatically.
