...
- For operations that take time, show a [Loader](?path=/docs/components-loader--docs) or [Progress Bar](?path=/docs/components-progress-bar--docs) so the user knows work is in progress.
- For completed actions that do not require a new page or view, use a [Snackbar](?path=/docs/components-snackbar--docs) to confirm the outcome without interrupting the flow.
