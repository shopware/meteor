...
<StorybookPageHeader status="none" title="Migration" />
...
Replace custom CSS variables or hard-coded colors with [Meteor tokens](?path=/docs/foundations-tokens-overview--docs):
...
The component name is the same. The tag just changes from `sw-icon` to `mt-icon`. See the [Icons](?path=/docs/foundations-icons--docs) page for the full icon set and usage guidance.
