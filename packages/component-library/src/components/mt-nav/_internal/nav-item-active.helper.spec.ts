...
describe("mt-nav/_internal/nav-item-active.helper", () => {
  describe("getActiveRouteNames", () => {
    ...
    it("ignores records without a name", () => {
      const route = {
        matched: [{ name: "core" }, {}, { name: undefined }, { name: "sw.product.index" }],
      };
  ...
  describe("itemParamsMatchRoute", () => {
    it("matches when the item declares no params", () => {
      expect(itemParamsMatchRoute({ path: "sw.product.index" }, { params: {} })).toBe(true);
    });

    it("matches only when every declared param equals the route param", () => {
      const route = { params: { id: "abc" } };

      expect(itemParamsMatchRoute({ params: { id: "abc" } }, route)).toBe(true);
      expect(itemParamsMatchRoute({ params: { id: "xyz" } }, route)).toBe(false);
    });

    it("compares params as strings (uuid / numeric parity)", () => {
      expect(itemParamsMatchRoute({ params: { id: 5 } }, { params: { id: "5" } })).toBe(true);
    });
  });

  describe("isItemOnActiveRoute", () => {
    it("lights a leaf whose route is in the matched chain", () => {
      const route = { matched: [{ name: "sw.product.index" }], params: {} };

      expect(isItemOnActiveRoute({ path: "sw.product.index" }, route)).toBe(true);
    });

    it("does not light a leaf whose route is not in the matched chain", () => {
      const route = { matched: [{ name: "sw.order.index" }], params: {} };

      expect(isItemOnActiveRoute({ path: "sw.product.index" }, route)).toBe(false);
    });
    ...
    it("lights a path-less parent when a descendant route is in the matched chain", () => {
      ...
      expect(isItemOnActiveRoute(extensions, route)).toBe(true);
    });
    ...
    it("does not light a path-less parent when no descendant is active", () => {
      ...
      expect(isItemOnActiveRoute(extensions, route)).toBe(false);
    });

    it("disambiguates items that share a route name by their params", () => {
      ...
      expect(isItemOnActiveRoute(entryA, route)).toBe(true);
      expect(isItemOnActiveRoute(entryB, route)).toBe(false);
    });
    ...
    it("lights a leaf reachable only through the parentPath bridge", () => {
      ...
      expect(isItemOnActiveRoute({ path: "sw.product.index" }, route, activeNames)).toBe(true);
    });
  });
});
