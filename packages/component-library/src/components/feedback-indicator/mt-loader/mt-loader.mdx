...
<Canvas of={LoaderStories.WithText} />
...
- Use `headline` and `description` when users need short context for what is loading.
...
- `headline` and `description` render below the spinner as centered body text and can be used independently.

## Accessibility notes

- **Loader** is visual only and does not announce loading state to assistive technologies on its own.
- `headline` and `description` provide visible context, but the surrounding region should still expose an appropriate busy state when the loading state matters for screen-reader users.
