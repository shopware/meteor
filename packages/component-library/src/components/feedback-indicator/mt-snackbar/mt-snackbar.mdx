...
>
  **Snackbar** displays temporary overlay feedback near the bottom-right corner
  of the interface. Use it to confirm an action, report a lightweight outcome,
...
- Use it when you may need one lightweight follow-up action, such as `Undo`.
...
**Snackbar** is usually used as a pair:
...
- The optional `link` supports one lightweight follow-up action, not a full decision flow.
