...
When switching from Code → WYSIWYG, the editor performs a dry-run parse with TipTap and compares the HTML. If changes are detected (e.g., unsupported attributes removed), a modal shows a side-by-side diff. You can accept to switch with the modified HTML or stay in code mode.
