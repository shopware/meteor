...
export const PreserveBasicHTMLStructure: MtTextEditorStory = defineStory({
  ...
  args: {
    modelValue: '<p>This is a <span class="highlight">highlighted</span> text.</p><p>Another paragraph.</p>',
  },
  play: async ({ canvasElement, args }) => {
    const canvas = within(canvasElement);
...
export const PreserveImageTags: MtTextEditorStory = defineStory({
  ...
  args: {
    modelValue: '<p>Here is an image:</p><img class="responsive-img" style="max-width: 100%;" src="https://placehold.co/200x50" alt="Test image"><p> and some text.</p>',
  },
  play: async ({ canvasElement, args }) => {
    const canvas = within(canvasElement);
    ...
    const expectedHtml = '<p>Here is an image:</p><img src="https://placehold.co/200x50" alt="Test image" class="responsive-img" style="max-width: 100%;"><p><strong>and some text.</strong></p>';
...
export const PreserveLinkAttributesInternal: MtTextEditorStory = defineStory({
  ...
  args: {
    modelValue: '<p>Visit our <a href="/internal-page" target="_self">internal page</a> for more info.</p><p>Some additional text.</p>',
  },
  play: async ({ canvasElement, args }) => {
    const canvas = within(canvasElement);
...
export const PreserveLinkAttributesUnknown: MtTextEditorStory = defineStory({
  ...
  args: {
    modelValue: '<p>Visit our <a href="/normal-link">normal link</a> for more info.</p><p>Some additional text.</p>',
  },
  play: async ({ canvasElement, args }) => {
    const canvas = within(canvasElement);
...
export const PreserveLinkAttributesExternal: MtTextEditorStory = defineStory({
  ...
  args: {
    modelValue: '<p>Visit our <a target="_blank" href="/external-link">external link</a> for more info.</p><p>Some additional text.</p>',
  },
  play: async ({ canvasElement, args }) => {
    const canvas = within(canvasElement);
...
export const PreserveSpanTags: MtTextEditorStory = defineStory({
  ...
  args: {
    modelValue: '<p>This is <span class="highlight" style="background-color: yellow;">highlighted text</span> in a paragraph.</p><p>Another paragraph.</p>',
  },
  play: async ({ canvasElement, args }) => {
    const canvas = within(canvasElement);
...
export const PreserveDivTags: MtTextEditorStory = defineStory({
  ...
  args: {
    modelValue: '<div class="container" id="foo" style="max-width: 900px; margin: 0 auto;"><h2>Title</h2><p>Content goes here.</p></div><p>Additional text.</p>',
  },
  play: async ({ canvasElement, args }) => {
    const canvas = within(canvasElement);
...
export const PreserveSemanticElements: MtTextEditorStory = defineStory({
  ...
  args: {
    modelValue: '<header class="page-header"><nav class="main-nav"><ul><li>Home</li><li>About</li></ul></nav></header><main><p>Main content here.</p></main><footer class="page-footer"><p>Footer content.</p></footer><p>Some additional text.</p>',
  },
  play: async ({ canvasElement, args }) => {
    const canvas = within(canvasElement);
...
export const PreserveFigureAndFigcaption: MtTextEditorStory = defineStory({
  ...
  args: {
    modelValue: '<figure class="image-figure" style="text-align: center;"><img src="/chart.png" alt="Sales Chart" /><figcaption>Sales data for Q1 2024</figcaption></figure><p>Analysis below.</p>',
  },
  play: async ({ canvasElement, args }) => {
    const canvas = within(canvasElement);
...
export const PreserveCSSClasses: MtTextEditorStory = defineStory({
  ...
  args: {
    modelValue: '<div class="content-wrapper custom-layout"><h2 class="section-title primary-color">Section Title</h2><p class="text-content large-text">This is important content.</p></div>',
  },
  play: async ({ canvasElement, args }) => {
    const canvas = within(canvasElement);
...
export const PreserveComplexHTMLStructure: MtTextEditorStory = defineStory({
  ...
  args: {
    modelValue: '<div style="max-width: 900px; margin: 0 auto;"><h2 class="display-5" style="text-align: center; margin-bottom: 2rem;">Placeholder Headline Text</h2></div><div style="max-width: 900px; margin: 0 auto;"><p style="font-size: 1rem; text-align: center;">Placeholder paragraph text with a <a href="/sample-link" target="_self">sample link</a> and more placeholder text to demonstrate formatting effects.</p></div>',
  },
  play: async ({ canvasElement, args }) => {
    const canvas = within(canvasElement);

    // Click on the paragraph text containing "placeholder"
    await userEvent.click(canvas.getByText("placeholder"));

    // Select the "placeholder" word in the paragraph
    selectText(canvas.getByText("placeholder"));
    ...
    // Check that the complex structure is preserved
    const expectedHtml = args.updateModelValue.mock.calls[0][0];
    
    // Verify div containers with styles are preserved
    expect(expectedHtml).toContain('<div style="max-width: 900px; margin: 0 auto;">');
    
    // Verify h2 with class and styles are preserved
    expect(expectedHtml).toContain('<h2 class="display-5" style="text-align: center; margin-bottom: 2rem;">');
    
    // Verify paragraph with styles is preserved
    expect(expectedHtml).toContain('<p style="font-size: 1rem; text-align: center;">');
    
    // Verify link with target="_self" doesn't get unwanted attributes
    expect(expectedHtml).toContain('<a href="/sample-link" target="_self">sample link</a>');
    expect(expectedHtml).not.toContain('rel="noopener noreferrer nofollow"');
    
    // Verify the formatted text
    expect(expectedHtml).toContain("<strong>placeholder</strong>");
  },
});
