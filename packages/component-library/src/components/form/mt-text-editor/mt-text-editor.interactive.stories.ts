...
export const SetLink: MtTextEditorStory = {
  ...
  play: async ({ canvasElement, args }) => {
    ...
    // Toggle link target
    const targetCheckbox = document.querySelector("div[aria-label='Open in new tab'] input[type='checkbox'") as HTMLInputElement;
...
export const VisualTestShowContextualButtons: MtTextEditorStory = {
  ...
  play: async ({ canvasElement, args }) => {
    ...
    const tableCell = document.querySelector('.mt-text-editor__content-editor table td') as HTMLElement;
